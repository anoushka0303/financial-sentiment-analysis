...
from config import OPENAI_API_KEY, VECTOR_DB_PATH, COLLECTION_NAME, EMBEDDING_MODEL, LLM_MODEL, TICKERS, COMPANIES
import os
...
def get_latest_stock_data(company, days=30):
    ...
    try:
        ...
        # Add sentiment column (for now, use neutral sentiment)
        data['sentiment'] = 0.0
...
def load_stock_model(company):
    ...
    try:
        ...
        input_dim = 4  # Open, High, Low, Volume (no sentiment for now)
...
def predict_stock_price(company, recent_features):
    ...
    try:
        ...
        # Convert to tensor
        seq_tensor = torch.tensor(features_normalized, dtype=torch.float).unsqueeze(0).to(device)
...
def summarize_financial_data(company, query_date=None):
    ...
    try:
        ...
        if stock_data.empty:
            return f"No stock data available for {company}"
